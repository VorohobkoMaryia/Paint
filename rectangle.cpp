#include "rectangle.h"

Rectangle::Rectangle(double x, double y, double w, double h)
    : x(std::min(x, x + w)), y(std::min(y, y + h)),
      width(fabs(w)), height(fabs(h)) {
  type = rectangle_t;
}

void Rectangle::Draw(QPainter& painter) {
  double x_coeff = scale / max_x * painter.window().width();
  double y_coeff = scale / max_y * painter.window().height();
  painter.translate((x + width / 2) / max_x * painter.window().width(),
                    (y + height / 2) / max_y * painter.window().height());
  painter.rotate(angle);
  painter.drawRect(lround(-width / 2 * x_coeff), lround(-height / 2 * y_coeff),
                   lround(width * x_coeff), lround(height * y_coeff));
  painter.rotate(-angle);
  painter.translate(-(x + width / 2) / max_x * painter.window().width(),
                    -(y + height / 2) / max_y * painter.window().height());
}

void Rectangle::MoveBy(const QPointF& point) {
  x += point.x();
  y += point.y();
}
